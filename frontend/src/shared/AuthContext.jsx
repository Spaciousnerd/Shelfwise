...
export const AuthProvider = ({ children }) => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (data.success && data.user) {
              ...
              console.log("Restored user:", mappedUser);
            ...
            console.log("Calling logout because response was not OK");
            logout();
          }
        } catch (error) {
          console.error(
  ...
  const login = async ({ email, password, role }) => {
    try {
      ...
      return {
        ok: false,
        error: `Server connection failed. Please ensure the backend is running on ${API_URL}`,
      };
    }
  };
  const logout = () => {
  ...
  // regiter account and trigger otp
...
export const useAuth = () => {
  ...
  if (!context) throw new Error("UseAuth must be inside provider");
  return context;
};
