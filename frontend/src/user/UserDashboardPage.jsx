        ...
        <div className={s.heroGrid}>
          <div className={s.heroLeft}>
            ...
            <h1 className={s.heroTitle}>
              {currentUser?.name ?? "reader"} : Your profile, semester status,
              and your latest library books
            </h1>
          ...
          <div className={s.rightColumnGrid}>
            <article className={s.profileCard}>
              ...
              <div className={s.profileDetails}>
                <div className={s.profileDetailItem}>
                  StudentId : {currentUserSummary?.studentId ?? "Not assigned"}
                </div>
                <div className={s.profileDetailItem}>
                  Roll Number :{" "}
            ...
            <article className={s.semesterCard}>
              ...
              <div className={s.semesterDetails}>
                ...
                <div className={s.semesterDetailItem}>
                  AcademicYear: {currentUserSummary?.academicYear ?? "1st Year"}
                </div>
              </div>
            </article>
          </div>
          ...
          {recentBooks.length ? (
            recentBooks.map((record) => (
              <UserBookCard
                key={record.id}
                record={record}
                borrowedName={currentUser?.name ?? "student"}
              />
            ))
          ) : (
